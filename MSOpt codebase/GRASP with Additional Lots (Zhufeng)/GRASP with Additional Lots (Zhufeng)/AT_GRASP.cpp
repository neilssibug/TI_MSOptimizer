				 ...
				 )
{
	...
	while (!whether_stop)
	{
		...
		if( whether_new_lot )
		{
			...
			int type =1;
			cout<<"type = "<<type<<endl;
			assign_lot_reset(reset_machine_list,candidate_lot_list,finished_lot_reset_list,current_time,tool_setup_list,mf_tool_setup_list, tf_cert_ti_list, type);
...
//Feb 24
void AT::assign_lot_reset( vector<multi_mach> & machine_list, //note: when the function is used, this should be reset_machine_list
		             vector<multi_lot> & candidate_lot_list,
					 vector<multi_lot> & finished_lot_reset_list,
					 double current_time,
					 const vector<tool_setup> tool_setup_list,
