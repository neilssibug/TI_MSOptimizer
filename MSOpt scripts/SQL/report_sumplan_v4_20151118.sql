...
select * from wip where device = 'TPS54061DRBR-C';

select * from solution s inner join wip on wip.device = s.device_name and s.lot_name = wip.lot_name;

select sum(quantity) from solution where Machine_Instance = 'CLFTEA15';