 /*
 date modified: 2015-12-08
 details:
 corrected the format for machine instance from smsdw
 add this in the script 
		- "coalesce(trim(case when substr(t.tester_TWSTATUS_i,7,1)=0 then 
			concat('CLFTEA',substr(t.tester_TWSTATUS_i,8,1)) else t.tester_TWSTATUS_i end ),'') as machine_instance"
 */
 ...
 UNION select sp_pph.MI, sp_pph.MF, sp_pph.PKG_SIZE, sp_pph.LB as LB,    
                                 sp_pph.TF, sp_pph.NROWS, sp_pph.SUM, sp_pph.RNUM, sp_pph.SUM_PPH,    
                                 round(sum(sol.SUM_PPH),2) as APPROX,    
 ...
 from (select sp.MI, sp.MF, sp.PKG_SIZE, sp.LB, sp.TF, sp.NROWS, sp.SUM, sp.RNUM, r.SUM_PPH, pin    
 		...
 		left join    
 		(select sol.package,sol.Machine_Family_name,sol.Tooling_Family_name,
sum(time_to_sec(timediff(date_format(str_to_date(replace(completion_time,'00','12'),'%m/%d/%Y %H:%i'), '%Y/%m/%d %H:%i:%s'),date_format(case when substr(trim(cast(Start_Time as char )),1,2)='0/' 
then str_to_date(concat('12/',substr(trim(cast(Start_Time as char )),3,18)),'%m/%d/%Y %H:%i') end, '%Y/%m/%d %H:%i:%s'))) /3600) as SUM_PPH
from solution sol
group by sol.package, sol.Machine_Family_name, sol.Tooling_Family_name 
 		) sol on sol.Package = sp.PKG_SIZE and sol.Machine_Family = sp.MF and sol.Tooling_Family = sp.TF    
 ) sp_pph    
 left join    
 ...
 ) so on trim(so.machine_instance) = trim(sp_pph.MI) and so.EDGE_LB = sp_pph.TF and so.pkg_size = sp_pph.PKG_SIZE     
 ;
