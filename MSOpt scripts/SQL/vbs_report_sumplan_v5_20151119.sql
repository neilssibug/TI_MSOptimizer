...
from (select sp.MI, sp.MF, sp.PKG_SIZE, sp.LB, sp.TF, sp.NROWS, sp.SUM, sp.RNUM, r.SUM_PPH, pin
                from (select MI, MF, PKG_SIZE, LB, TF, NROWS, SUM,
						case when MI = @prev_MI and (pkg_size <> @prev_pkg_size or tf <> @prev_tf ) then (@rownum := @rownum + 1) else @rownum := 0 end as RNUM, pin,
						@prev_MI := MI,@prev_pkg_size := PKG_SIZE, @prev_tf := TF
		...
		from (select s.Machine_Instance, s.Machine_Family_Name, s.Package, s.Tooling_Family_Name, s.Quantity,s.device_name, pin
...
group by sol.machine_instance, pkg.pin, pkg.pkg_size,sol.Tooling_Family_Name
